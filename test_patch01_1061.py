...
class Patch011061Tests(unittest.TestCase):
    ...
    def test_source_tree_managed_history_marks_same_name_edit_user_modified(self):
        with tempfile.TemporaryDirectory() as td:
            target=Path(td)/'config'/'rules';target.mkdir(parents=True)
            name='shiga_2026.txt'
            bundled=(ROOT/'config/rules'/name).read_bytes()
            (target/name).write_bytes(bundled+b'\n')
            state={'schema':1,'managed':{name:'known-managed-hash'}}
            (target/'.pslog-bundled-state.json').write_text(json.dumps(state),encoding='utf-8')
            self.assertEqual(bundled_status(target,'config/rules',name),'user_modified')

    def test_frozen_same_name_without_state_keeps_legacy_user_modified(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                base=Path(td);bundle=base/'bundle';target=base/'data'/'rules';source=bundle/'config'/'rules'
                source.mkdir(parents=True);target.mkdir(parents=True)
                (source/'a.txt').write_text('official\n',encoding='utf-8')
