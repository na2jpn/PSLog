"""Create a checked handoff for party/award implementation through step 8."""
...
def checked_tests(logpath):
    ...
    if count!=566:
        raise ValueError(f'expected 566 tests, got {count}')
    return count
...
def package(logpath):
    ...
    report=(ROOT/'docs/CHECKPOINT_PSLOG101_ACTIVITY_STAGE8.md').read_text()
    outputs=[]
    for index,batch in enumerate(groups(files)):
        name='PSLOG101_activity_stage8_source.zip' if index==0 else f'PSLOG101_activity_stage8_references_{index:02}.zip'
        target=HANDOFF/name
        fd,tempname=tempfile.mkstemp(prefix='pslog-stage8-',suffix='.zip',dir='/tmp');os.close(fd);temp=Path(tempname)
        try:
    ...
    check={'created':'2026-09-15','stage':'party-award-8-of-12','tests':tests,'files':len(files),'manifest_entries':len(manifest),'archives':outputs,'pending_steps':[9,10,11,12]}
    check_path=HANDOFF/'PSLOG101_activity_stage8_package_checks.json'
